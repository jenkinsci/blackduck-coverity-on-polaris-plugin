...
public class PolarisCliArgumentServiceTest {
    ...
    private static Stream<Arguments> getWindowsArgumentsAndExpected() {
        return Stream.of(
                ...
                Arguments.of(
                        Arrays.asList("--co", "project.name=\"newProjectname\"", "analyze"),
                        Arrays.asList(POLARIS_CLI_PATH, "--co", "project.name=\\\"newProjectname\\\"", "analyze")));
    }
    ...
    public void testFinalizePolarisArgumentsLinux(
            List<String> tokenizedPolarisArguments, List<String> expectedFinalizedPolarisArguments) {
        ...
        List<String> finalizedPolarisArguments = polarisCliArgumentService.finalizePolarisCliArguments(
                OperatingSystemType.LINUX, POLARIS_CLI_PATH, tokenizedPolarisArguments);
    ...
    @ParameterizedTest
    @MethodSource("getUnixArgumentsAndExpected")
    public void testFinalizePolarisArgumentsMac(
            List<String> tokenizedPolarisArguments, List<String> expectedFinalizedPolarisArguments) {
        JenkinsIntLogger logger = Mockito.mock(JenkinsIntLogger.class);

        PolarisCliArgumentService polarisCliArgumentService = new PolarisCliArgumentService(logger);
        List<String> finalizedPolarisArguments = polarisCliArgumentService.finalizePolarisCliArguments(
                OperatingSystemType.MAC, POLARIS_CLI_PATH, tokenizedPolarisArguments);
    ...
    public void testFinalizePolarisArgumentsWindows(
            List<String> tokenizedPolarisArguments, List<String> expectedFinalizedPolarisArguments) {
        JenkinsIntLogger logger = Mockito.mock(JenkinsIntLogger.class);

        PolarisCliArgumentService polarisCliArgumentService = new PolarisCliArgumentService(logger);
        List<String> finalizedPolarisArguments = polarisCliArgumentService.finalizePolarisCliArguments(
                OperatingSystemType.WINDOWS, POLARIS_CLI_PATH, tokenizedPolarisArguments);
