...
public class PolarisGlobalConfigTest {
    ...
    private static final String CONFIG_XML_CONTENTS = "<?xml version='1.1' encoding='UTF-8'?>\n"
            ...
            + "  <polarisUrl>https://dev01.dev.polaris.blackduck.com</polarisUrl>\n"
    ...
    public void testInvalidUrl() {
        ...
        FormValidation formValidation =
                detectGlobalConfig.doTestPolarisConnection(INVALID_URL, POLARIS_CREDENTIALS_ID, POLARIS_TIMEOUT_STRING);
    ...
    public void testInvalidCredentialsId() {
        ...
        PolarisGlobalConfig detectGlobalConfig = new PolarisGlobalConfig();
        FormValidation formValidation =
                detectGlobalConfig.doTestPolarisConnection(POLARIS_URL, POLARIS_CREDENTIALS_ID, POLARIS_TIMEOUT_STRING);
    ...
    public void testValidConfig() {
        ...
        Mockito.when(mockedServerConfigBuilder.setTimeoutInSeconds(POLARIS_TIMEOUT_INT))
                .thenReturn(mockedServerConfigBuilder);
        ...
        Mockito.when(mockedServerConfig.createPolarisHttpClient(Mockito.any(IntLogger.class)))
        ...
        Mockito.when(polarisGlobalConfig.createPolarisServerConfigBuilder(
                        Mockito.any(),
                        Mockito.any(),
                        Mockito.eq(POLARIS_URL),
                        Mockito.eq(POLARIS_CREDENTIALS_ID),
                        Mockito.eq(POLARIS_TIMEOUT_INT)))
                .thenReturn(mockedServerConfigBuilder);
        Mockito.when(polarisGlobalConfig.doTestPolarisConnection(
                        POLARIS_URL, POLARIS_CREDENTIALS_ID, POLARIS_TIMEOUT_STRING))
                .thenCallRealMethod();

        FormValidation formValidation = polarisGlobalConfig.doTestPolarisConnection(
                POLARIS_URL, POLARIS_CREDENTIALS_ID, POLARIS_TIMEOUT_STRING);
    ...
    public void testConfigDotXmlGet() throws ServletException, ParserConfigurationException, IOException {
        ...
        ServletOutputStream servletOutputStream =
                new FilterServletOutputStream(byteArrayOutputStream, Mockito.mock(ServletOutputStream.class));
        Mockito.when(rsp.getOutputStream()).thenReturn(servletOutputStream);

        File pluginConfigFile =
                new File(Jenkins.getInstance().getRootDir(), PolarisGlobalConfig.class.getName() + ".xml");
